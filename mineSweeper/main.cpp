...
void getInput(char &condition, int &column, int &row) {
    std::cout << "Enter cell (r/f x y): ";

    std::cin >> condition >> column >> row;
    if (std::cin.fail()) 
    {
        std::cin.clear();
        std::cin.ignore(1000, '\n');
    ...
    for (int i = 0; i < GRID_SIZE; ++i) 
    {
        ...
        for (int j = 0; j < GRID_SIZE; ++j)
        {
            if (grid[i][j].isreveal()) 
            {
                ...
                {
                    int adj = grid[i][j].getAdjacentMines();
                    if (adj == 0)
                        std::cout << "[-]";
                    else 
...
int revealCells(std::vector<std::vector<Cell>>& grid, int moves, int column, int row) 
{
